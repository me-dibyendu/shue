...
const Request = (props) => {
  const [filterRequet, setFilterRequest] = useState('all')
    ...
    return (
      <View style={styles.productCard}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', }}>
          <Text style={[styles.info, { flex: 0.6, padding: 12.5, marginTop: 12.9, }]}>Waiting For Confrimation</Text>
          <Pressable onPress={chatToReseller} style={[styles.button, { flex: 0.4 }]}><Text style={styles.btnText}>Chat To Reseller</Text></Pressable>
        </View>
      </View>
    )
  }
  ...
  return (
    <View style={{ flex: 1, }}>
      ...
      <View style={{ flexDirection: 'row', padding: Constants.padding, paddingBottom: 0, justifyContent: 'space-between', }}>
        <Pressable onPress={() => filterRequest('all')} style={{ ...styles.tabOrder, backgroundColor: filterRequet == 'all' ? Constants.colors.clock : '#FFF', }}><Text style={{ ...styles.tabText, color: filterRequet == 'all' ? Constants.colors.white : Constants.colors.primary, }}>All</Text></Pressable>
